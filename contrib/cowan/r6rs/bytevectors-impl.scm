...
;;;
;;; Note by John Cowan:  Cond-expand is not supported consistently
;;; in R6RS systems, so I have removed the uses of it in this version
;;; (packaged with SRFI 160) in favor of a manually changeable define.
;;; This introduces a tiny inefficiency into calls on the IEEE native
;;; procedures which an optimizing compiler should be able to remove.
;;; However, unlike other implementations, big-endian and little-endian
;;; systems are treated equally.
;;;
;;; Almost all current systems are little-endian, so that is the
;;; default here.  However, the choice of endianness is important
;;; if bytevectors are shared with non-Scheme code via the FFI
;;; and native integers or floats are present, so it should be
;;; changed if running on SPARC, IBM z/Architecture, or various
;;; obsolete CPU architectures.
;;;
...
;; CHANGE THIS on big-endian CPU architectures
(define native-endness 'little)
(define (native-endianness) native-endness)
...
;;;
;;; Note by John Cowan: This interpretation was adopted by an erratum
;;; to the R6RS
